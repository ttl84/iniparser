	...
	struct bnode * par, unsigned pos)
{
	...
	memcpy(items(bt, par, pos), items(bt, cur, bt->deg / 2 - 1), bt->ele_size);
...
int btree_remove(struct btree * bt, void const * item)
{
	...
	while(1)
	{
		...
		/* get one more key to become half full*/
	...
	if(targ_cur)
	{
		...
		if(cur == targ_cur)
			...
			memcpy(items(bt, targ_cur, targ_pos), items(bt, cur, pos - 1),
				bt->ele_size);
		cur->size--;
		/* decrease height*/
		if(bt->root->size == 0 && bt->root->in)
		{
